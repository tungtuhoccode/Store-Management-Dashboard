        ...
        <header className="fixed top-0 left-0 w-full z-40 transition-all duration-300 bg-white text-black shadow-md border-b">
            <div className="max-w-7xl mx-auto flex justify-between items-center px-4 md:px-8 py-6">
                ...
                <ul className="hidden md:flex space-x-6 items-center">
                    <li className='hover:text-green-500'><Link to="/" className={`hover:text-green-500 ${currentLocation === '/' ? 'text-green-600' : 'text-black'}`} >Home</Link></li>
            ...
            {/* Mobile Nav Panel */}
            {isMenuOpen && (
                <div className="md:hidden bg-white px-6 pb-6 pt-4 shadow-lg">
                    <ul className="space-y-4">
                        ...
                        {user ?
                            ...
                            :
                            <li className='hover:text-green-500' onClick={() => setIsMenuOpen(false)}><Link to="/signup" className={`hover:text-green-500 ${currentLocation === '/signup' ? 'text-green-600' : 'text-black'} flex justify-start items-center gap-1`} ><User /> Sign up</Link></li>
                        }
                    </ul>
                </div>
            )}
